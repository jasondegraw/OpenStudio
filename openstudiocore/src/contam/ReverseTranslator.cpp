...
static Point3d transform(Icon *icon, int originX, int originY, double z, double scale, bool invertY)
{
  if(invertY)
...
boost::optional<openstudio::model::Model> ReverseTranslator::translate(const IndexModel &data, ProgressBar *progressBar)
{
  m_logSink.setThreadId(QThread::currentThread());
...
void ReverseTranslator::translateLevels(openstudio::model::Model &model, const IndexModel &data)
{
  ...
  for(unsigned i=0;i<data.zones().size();i++)
  {
  ...
  for(unsigned k = 0; k<data.levels().size(); k++)
  {
    if(counts[k])
    {
      model::BuildingStory story(model);
      story.setName(data.levels()[k].name());
      story.setNominalZCoordinate(data.levels()[k].refht());
      story.setNominalFloortoFloorHeight(data.levels()[k].delht());
...
bool ReverseTranslator::translateZones(openstudio::model::Model &model, const IndexModel &data)
{
  if(!data.valid())
  {
    return false;
...
bool ReverseTranslator::translateSimpleAHS(openstudio::model::Model &model, const IndexModel &data)
{
  if(!data.valid())
  {
    return false;
  ...
  for(unsigned k = 0; k<data.levels().size(); k++)
  {
    for(unsigned j = 0; j<data.levels()[k].icons().size(); j++)
    {
      if(data.levels()[k].icons()[j].icon() == BOX_S)
      {
        int path = data.levels()[k].icons()[j].nr();
        int fromZone = data.airflowPaths()[path-1].pzn();
        int toZone = data.airflowPaths()[path-1].pzm();
        ...
        int ahsIndex = -1;
        for (unsigned i = 0; i<data.ahs().size(); i++)
        {
  ...
  for(unsigned k = 0; k<data.ahs().size(); k++)
  {
    if(counts[k])
    {
  ...
  for(unsigned i = 1; i<data.zones().size(); i++) // Skip the fake ambient zone
  {
    if(m_connections.contains(data.zones()[i].nr()))
    {
      ...
      {
        // Error!
        LOG(Error, "Unable to find airloop or thermal zone for airflow zone '" << data.zones()[i].name() << "'.");
        return false;
      }
